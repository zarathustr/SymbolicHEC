...
function symmetric_variable_matrix(x::Convex.IndexAtom)
...
function skew_variable_matrix(x::Convex.IndexAtom)
    X = sparse([0 -0.5 0; 0.5 0 0; 0 0 0])*x[3] + sparse([0 0 0.5; 0 0 0; -0.5 0 0])*x[2] + sparse([0 0 0; 0 0 -0.5; 0 0.5 0])*x[1]
    return X
end

function generate_n_rotation_dual_constraints_using_concat_at_i(n::Int64, scale::Bool, usecolumnorthogonality::Bool=true, usehandedness::Bool=true)
    ν_r =Variable(6 * n)
...
function generate_base_ortho_rows(x::Convex.IndexAtom)
    X = symmetric_variable_matrix(x)
    return kron(sparse(Matrix(-I, 3, 3)), X)
end

function generate_base_ortho_cols(x::Convex.IndexAtom)
    X = symmetric_variable_matrix(x)
    return kron(X, sparse(Matrix(-I, 3, 3)))
end

function generate_base_handedness(x::Convex.IndexAtom)
    h_123 = x[1:3]
    h_231 = x[4:6]
    h_312 = x[7:9]
